<template>
   <div>
    ...
    <el-dialog title="查看" :visible.sync="dialogVisible" width="70%">
      <Table v-if="hackReset" :message="table"></Table>
    </el-dialog>
  </div>
</template>
<script>
...
import { getPer,getPerType } from "@/api/statistics"
...
export default {
  ...
  methods: {
    ...
    showPeddingInfo(row) {
      ...
      this.table = { state: row.table_state,tableId:row.table_id,type: 2 };
    ...
    search() {
      ...
      getPer({ resform: resform }).then(res => {
