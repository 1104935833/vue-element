      ...
      <el-row>
        ...
        <el-col :span="12" :offset="2">
          <el-form-item label="所在院系：" prop="departmentPlace">
            <el-input v-model="form.departmentPlace" placeholder="请输入内容" :disabled="disable" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="10">
          <el-form-item label="专业负责人：" prop="professionalLeader">
            <el-input v-model="form.professionalLeader" placeholder="请输入内容" :disabled="disable" />
          </el-form-item>
        </el-col>
        <el-col :span="12" :offset="2">
...
import { getUserRole, getFileNameById, check, delFile,getProfessional,updataProfessional,insertProfessional } from "@/api/achievementinput";
export default {
  props: ["table"],
  data() {
    return {
  ...
  mounted() {
    if (this.table != undefined && this.table != null) {
      getProfessional({ tableId: this.table.tableId }).then(res => {
  ...
  methods: {
    ...
    onSubmit(form) {
      this.$refs[form].validate(valid => {
        if (valid) {
          insertProfessional(this.form).then(res => {
            this.clear()
          })
        } else {
          return false
