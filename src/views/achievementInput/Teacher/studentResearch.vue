      ...
      <el-row>
        ...
        <el-col :span="12" :offset="2">
          <el-form-item label="工程负责人：" prop="projectLeader">
            <el-input v-model="form.projectLeader" placeholder="请输入内容" :disabled="disable" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="10">
          <el-form-item label="指导教师：" prop="tutor">
            <el-input v-model="form.tutor" placeholder="请输入内容" :disabled="disable" />
          </el-form-item>
        </el-col>
        <el-col :span="12" :offset="2">
...
import { getUserRole, getFileNameById, check, delFile,getStudentResearch,updataStudentResearch,insertStudentResearch } from "@/api/achievementinput";
export default {
  props: ["table"],
  data() {
    return {
  ...
  mounted() {
    if (this.table != undefined && this.table != null) {
      getStudentResearch({ tableId: this.table.tableId }).then(res => {
  ...
  methods: {
    ...
    onSubmit(form) {
      this.$refs[form].validate(valid => {
        if (valid) {
          insertStudentResearch(this.form).then(res => {
            this.clear()
          })
        } else {
          return false
