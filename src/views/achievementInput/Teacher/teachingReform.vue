...
import { getUserRole, getFileNameById, check, delFile,getTeachingReform,updataTeachingReform,insertTeachingReform } from "@/api/achievementinput";
export default {
  props: ["table"],
  data() {
    return {
...
if (this.table != undefined && this.table != null) {
      getTeachingReform({ tableId: this.table.tableId }).then(res => {
  ...
  methods: {
    ...
    onSubmit(form) {
      this.$refs[form].validate(valid => {
        if (valid) {
          insertTeachingReform(this.form).then(res => {
            this.clear()
          })
        } else {
          return false
