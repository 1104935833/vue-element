      ...
      <el-row>
        ...
        <el-col :span="12" :offset="2">
          <el-form-item label="专业学科：" prop="professionalSubject">
            <el-input v-model="form.professionalSubject" placeholder="请输入内容" :disabled="disable" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="10">
          <el-form-item label="负责人：" prop="leader">
            <el-input v-model="form.leader" placeholder="请输入内容" :disabled="disable" />
          </el-form-item>
        </el-col>
        <el-col :span="12" :offset="2">
...
import { getUserRole, getFileNameById, check, delFile,getSkill,updataSkill,insertSkill } from "@/api/achievementinput";
export default {
  props: ["table"],
  data() {
    return {
  ...
  mounted() {
    if (this.table != undefined && this.table != null) {
      getSkill({ tableId: this.table.tableId }).then(res => {
  ...
  methods: {
    ...
    onSubmit(form) {
      this.$refs[form].validate(valid => {
        if (valid) {
          insertSkill(this.form).then(res => {
            this.clear()
          })
        } else {
          return false
