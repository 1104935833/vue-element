      ...
      <el-row>
        <el-col>
          <el-form-item label="所属一级学科：" prop="firstLevel">
            <el-input v-model="form.firstLevel" placeholder="请输入..." :disabled="disable" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
      ...
      <el-row>
        <el-col>
          <el-form-item label="委托单位全称：" prop="clientName">
            <el-input v-model="form.clientName" placeholder="请输入..." :disabled="disable" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
        <el-col>
          <el-form-item label="委托单位所在地：" prop="clientAddress">
            <el-input v-model="form.clientAddress" placeholder="请输入..." :disabled="disable" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
...
import { getUserRole, getFileNameById, check, delFile,getTransverse,updataTransverse,insertTransverse } from "@/api/achievementinput";
export default {
  props: ["table"],
  data() {
    return {
  ...
  mounted() {
    if (this.table != undefined && this.table != null) {
      getTransverse({ tableId: this.table.tableId }).then(res => {
  ...
  methods: {
    ...
    onSubmit() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          insertTransverse(this.form).then(res => {
            this.clear()
          })
        } else {
          return false
