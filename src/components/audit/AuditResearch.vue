<template>
  <div>
    ...
    <el-dialog
      ...
      width="70%">
   <AuditCourts :message='message1'></AuditCourts>
</el-dialog>
   
  </div>
</template>
<script>
import AuditCourts from './AuditCourts';
export default {
  components: {
        'AuditCourts': AuditCourts  //将别名demo 变成 组件 Demo
      },
  data() {
    return {
      dialogVisible: false,
  ...
  methods: {
    searchEmp(){
      this.getRequest("/getAllAudit",{proposer_id:this.proposer_id,audit_status:this.audit_status,page:this.currentPage,size:10}).then(res => {
    ...
    getpeddingList() {
      this.getRequest("/getAllAudit",{proposer_id:'',audit_status:'',page:this.currentPage,size:10}).then(res => {
        this.totalCount = res.data.count;
        this.peddingList = res.data.audits;
      });
    }
