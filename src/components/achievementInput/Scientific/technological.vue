  ...
  data() {
    return {
      ...
      rules:{
        ...
        declareUnit: [{required:true,message:'请输入申报单位',trigger:'blur'}],
        planCategory: [{required:true,message:'请输入计划类别',trigger:'blur'}],
        specialName: [{required:true,message:'请输入专题名称',trigger:'blur'}],
        supportUnit: [{required:true,message:'请输入支持单位',trigger:'blur'}],
        declareTime: [{required:true,message:'请输入申报时间',trigger:'blur'}],
        organizationUnit: [{required:true,message:'请输入组织单位',trigger:'blur'}],
        address: [{required:true,message:'请输入联系地址',trigger:'blur'}],
        declareUnitName: [{required:true,message:'请输入申报单位名称',trigger:'blur'}],
        head: [{required:true,message:'请输入负责人',trigger:'blur'}],
        phone: [{required:true,message:'请输入联系方式',trigger:'blur'}],
        approvalDepartment: [{required:true,message:'请输入批准部门',trigger:'blur'}],
        approvalTime: [{required:true,message:'请输入批准时间',trigger:'blur'}],
      }
    };
  },
  mounted() {
    ...
    if (this.msgType != undefined) {
      if (this.msg.type == 2) {
        ...
        this.getRequest("/getTechnological", { id: this.msg.message.table_id }).then(
          res => {
