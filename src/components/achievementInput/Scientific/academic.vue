      ...
      <el-row>
        <el-col :span="24">
          <el-form-item label="团队人员：" prop="teamPersonnel">
            <el-input v-model="form.teamPersonnel" placeholder="请输入内容" :disabled="disable"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <!--第3行-->
      <el-row>
        <el-col :span="24">
          <el-form-item label="研究方向：" prop="direction">
            <el-input v-model="form.direction" placeholder="请输入内容" :disabled="disable"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <!--第4行-->
      <el-row>
        <el-col :span="24">
          <el-form-item label="所在学校：" prop="schoolPlace">
            <el-input v-model="form.schoolPlace" placeholder="请输入内容" :disabled="disable"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <!--第5行-->
      ...
      <el-row>
        <el-col :span="24">
          <el-form-item label="负责人联系电话：" prop="headPhone">
            <el-input v-model="form.headPhone" placeholder="请输入内容" :disabled="disable"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <!--第7行-->
      ...
      <el-row>
        <el-col :span="24">
          <el-form-item label="主管部门：" prop="competentDepartment">
            <el-input v-model="form.competentDepartment" placeholder="请输入内容" :disabled="disable"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
    ...
    if (this.msgType != undefined) {
      if (this.msg.type == 2) {
        this.form = {
          ...
          teamPersonnel: tableStatus.team_personnel,
          direction: tableStatus.direction,
          schoolPlace: tableStatus.school_place,
          address: tableStatus.address,
          headPhone: tableStatus.head_phone,
          declareTime: tableStatus.declare_time,
          competentDepartment: tableStatus.competent_department,
          level: tableStatus.level,
          fileId: tableStatus.file_id
        };
        this.disable = true;
      } else {
        this.getRequest("/getAcademic", { id: this.msg.message.table_id }).then(
          res => {
  ...
  methods: {
    ...
    onSubmit(form) {
      this.$refs[form].validate(valid => {
        if (valid) {
          this.postRequest("/insertAcademic", this.form).then(res => {
            this.clear();
          });
        } else {
          return false;
