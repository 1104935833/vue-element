<template>
  <div style="margin-top: 10px" v-loading="fullloading">
    ...
    <div style="display: flex;justify-content: space-around;flex-wrap: wrap;text-align: left">
      <el-card style="width: 350px;margin-bottom: 20px" v-for="(item,index) in users" :key="item.id"
  ...
  export default{
    ...
    methods: {
      ...
      refreshUser(userId, index){
        ...
        this.putRequest("/system/hr/id/" + userId).then(resp=> {
      ...
      deleteUser(userId){
        var _this = this;
        this.fullloading = true;
        this.deleteRequest("/system/hr/" + userId).then(resp=> {
          _this.fullloading = false;
          if (resp && resp.status == 200) {
            var data = resp.data;
